...
Social media and newspapers are flooded with biased, incompleted or misleading information. At this point everyone knows that each channle has its own agenda, political influence and a prefered framing of specific kind of situations. Unfortunelty people rarely explore or consume different perspective which make them unable to clearly identify these tactics and develop a critic point of view towards a event. 
...
> The idea is simple, cluster same-event coverage and rewrite them as a neutral article that keeps attributed reporting but drops loaded framing.
...
By combining multiple sources and separate claims from framing and then validate what is actually true.
...
How can we validate it? In this case is fairly straightforward, we simply need to know: 
...
I needed to establish a way keep the ingestion, preprocessing, audit and exposition independent from each other.

> **Ingenstion** -> **Preprocessing** -> **Audit** -> **Exposition**

**Ingestion**: A list of sources, "represented as classes" that are able to scrape the news  (using [crawl4ai](https://crawl4ai.com/)) every 3 hours and incrementally growing the local repository of news. Then, I added each article to Postgres and Chroma vector database (`news_index`) to be used for the preprocessing step.

**Preprocessing**: Groups those time-windowed articles in "clusters". These clusters represent the semantic relationship between articules, so we know that they talk about the same event. For this [agglomerative clustering](https://www.geeksforgeeks.org/machine-learning/agglomerative-clustering/) I use a distance threshold instead of a fixed `k` because the number of distinct events per window is unknown.
...
**Exposition**: A simple FastAPI backend and an MCP server over for agent clients.
...
1. We start with the **claim extractor**, which takes all these atomic claims and compiles a list of verifieable facts for the cluster.

2. Then the **fact-checker**, uses a trusted search tool to validate all the *claims* (whether through its own learned knowledge or by looking them up). This is not always possible to say, so the result could be one of these three: `supported`, `contraditect` or `inssuficient evidence` 

This *trusted search* is a [Serper.dev](https://serper.dev/) implementation that only searches trusted sources, such as such as domains `*.gob.do`, regional fact-checkers, UN/WHO-style orgs.

> Fact-checking itself is *constrained web search*, not vector RAG, so verification isn't circular.

3. While we're fact-checking claims, in parallel, another agent **Rhetoric Reviewer**, which checks the rhetoric within the text, so we can deetct loaded language and the framing intent and fallacies.

4. Now that we have everything in place, we can call the **Judge** agent, to determine what should be consider a fact and what should be explicitly debunked (it labels as `absolutely_false`, `not_verifiable`, and `narrative_to_keep` with attribution).

5. Another agent is in charge to **analyze** the results of that clsuter and what each outlet contributes to the truth. So we can get a sense of the agenda, or the kinds of topics, each outlet normally attacks or defends. 

6. Finally, a **Synthisizer** agent produces a new articule, drawing on the analysis and the judge's results to make clear statements about what should be reported. 
...
<h2 id="some-of-the-challenges-and-limitations">Some of the challeneges and limitations</h2>
...
> I don't have a published human-labeled benchmark yet. Informal review while building showed the a good performance but some expected failures (over-dropping topics, small clusters that look like "consensus" when they're just one wire story reprinted, and so on).

<h2 id="is-this-even-useful">Is this even useful then?</h2>

Absolutely, this is the first step of making society more transparent. But more than that, this serve as a based architecture for similar validation problems. We could build a similar version to determine whether a video is fake or not.

I've also enabled an MCP server so your agents can leverage the repository and access these news items and source scores.

You can find the MCP server here: [mcp-news.ogaston.com](https://mcp-news.ogaston.com/)
